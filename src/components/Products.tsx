...
export default function Products({ products, cart, setCart, onOpenProduct, onOpenCart, onToast }: ProductsProps) {
  ...
  const addToCart = (product: Product) => {
    ...
    onToast(` „  ≈÷«›… \"${product.name}\" ≈·Ï «·ÿ·»`, "success");
    setTimeout(() => {
      setAddedIds((prev) => {
    ...
    <section id="products" className="relative py-20 sm:py-28 bg-dark-900 noise-overlay">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.7 }} className="text-center mb-16">
          <span className="inline-block text-sm text-gold-400 tracking-widest mb-3 font-medium">«· ‘ﬂÌ·… «·„Œ «—…</span>
          <h2 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-bold text-beige-100 mb-4">„Ã„Ê⁄… «·⁄ÿÊ—</h2>
          <p className="text-beige-200/60 max-w-2xl mx-auto text-base leading-relaxed">»«ﬁ… „‰ «·⁄ÿÊ— «·‘—ﬁÌ… Ê«·›—‰”Ì… »ÿ«»⁄ ›«Œ—° „‰«”»… ··«” Œœ«„ «·ÌÊ„Ì Ê«·Âœ«Ì« Ê«·ÿ·»«  «·Œ«’….</p>
              ...
              <div className="bg-dark-800/80 border border-gold-400/20 rounded-sm p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm text-beige-200/80">„‰ Ã« ﬂ «·„Œ «—…: <span className="text-gold-400 font-bold">{cart.length}</span></div>
                      <div className="text-xs text-beige-200/50">{cart.map((p) => p.name).join("° ")}</div>
                    </div>
                  </div>
                  <button onClick={onOpenCart} className="btn-shine px-6 py-2.5 bg-gold-500 hover:bg-gold-400 text-dark-900 text-sm font-bold rounded-sm transition-colors whitespace-nowrap">⁄—÷ «·”·…</button>
                </div>
              </div>
            </motion.div>
          )}
              ...
              <TiltCard className="h-full">
                <div className="card-lift bg-dark-800/40 border border-gold-400/10 rounded-sm overflow-hidden hover:border-gold-400/25 transition-all duration-500 h-full flex flex-col">
                  <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-dark-700 via-dark-800 to-[#16110a] cursor-pointer flex items-center justify-center" onClick={() => onOpenProduct(product)}>
                    ...
                    <div className="absolute inset-0 bg-dark-900/35 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center"><span className="inline-flex items-center gap-2 px-4 py-2 bg-dark-900/80 border border-gold-400/30 text-gold-400 text-sm rounded-sm backdrop-blur-sm"><Eye className="w-4 h-4" />⁄—÷ «· ›«’Ì·</span></div>
                  ...
                  <div className="p-5 sm:p-6 flex flex-col flex-1">
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <div><span className="font-serif text-2xl font-bold text-gold-400">{product.price.toLocaleString("ar-SA")}</span><span className="text-sm text-beige-200/50 mr-1">—.”</span></div>
                      <button onClick={() => addToCart(product)} disabled={addedIds.has(product.id)} className={`btn-shine inline-flex items-center gap-2 px-4 py-2.5 rounded-sm text-sm font-bold transition-all duration-300 ${addedIds.has(product.id) ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-gold-500 hover:bg-gold-400 text-dark-900"}`}>
                        {addedIds.has(product.id) ? <><Check className="w-4 h-4" /> „  «·≈÷«›…</> : <><Plus className="w-4 h-4" />√÷› ··ÿ·»</>}
                      </button>
                    </div>
                  </div>
                </div>
